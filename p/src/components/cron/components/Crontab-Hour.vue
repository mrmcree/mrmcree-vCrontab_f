	...
	<el-form size="small">
		...
		<el-form-item>
			<el-radio v-model='radioValue' :label="3">
				从
				<el-input-number v-model='average01' :min="0" :max="24" /> 小时开始，每
				<el-input-number v-model='average02' :min="0" :max="24" /> 小时执行一次
			</el-radio>
		</el-form-item>
	...
	methods: {
		...
		radioChange() {
			if (this.radioValue === 1) {
				...
				if (this.cron.min === '*') {
					this.$emit('update', 'min', '*', 'hour');
				}
				if (this.cron.second === '*') {
					this.$emit('update', 'second', '*', 'hour');
				}
			}
